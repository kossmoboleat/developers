...
class ServiceDetails extends Component {
  ...
  render () {
    ...
    const { serviceName, serviceDescription, cancelModal, globalCheckbox, location } = this.state
    return (
      <Wrapper>
                      ...
                      <form>
                        ...
                        <Query query={GET_REGIONS}>
                          {({ loading, error, data }) => {
                            ...
                            let regionOptions = data.regions.nodes.map(function (node){return {value: node.id, label: node.name}})
                            return (
                              <Select
                                id='location'
